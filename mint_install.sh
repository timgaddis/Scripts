...
sudo sh -c 'echo "deb [arch=amd64] http://download.virtualbox.org/virtualbox/debian bionic contrib" >> /etc/apt/sources.list.d/virtualbox-offical-source.list'
...
#sudo sh -c 'echo "deb http://downloads.plex.tv/repo/deb public main" >> /etc/apt/sources.list.d/plexmediaserver.list'
...
sudo apt-add-repository -y ppa:webupd8team/java                         # Java 8
...
sudo apt install -y nvidia-graphics-drivers-396 nvidia-settings ntfs-config mint-meta-codecs ttf-mscorefonts-installer fonts-droid-fallback ckb-next gparted
...
sudo apt install -y pan gpodder steam qbittorrent hwinfo font-manager virtualbox-5.2 adobe-flashplugin grub-customizer plank curl deja-dup conky-all sublime-text jq keepassxc p7zip-full lynx xsltproc gimp gimp-plugin-registry gmic chromium-browser pepperflashplugin-nonfree google-chrome-stable plexmediaserver spideroakone menulibre
# backintime-qt4 gnome-encfs-manager nemo-dropbox stellarium inkscape hexchat gimp

print_title "Development apps"
sudo apt purge -y openjdk-8-jdk java-common
...
print_title "Themes"
sudo apt install -y numix-gtk-theme numix-icon-theme numix-icon-theme-circle numix-icon-theme-square papirus-icon-theme libreoffice-style-papirus moka-icon-theme faba-icon-theme paper-icon-theme gtk2-engines-murrine arc-theme
# vertex-theme system76-pop-gtk-theme system76-pop-icon-theme moka-gnome-shell-theme moka-gtk-theme
